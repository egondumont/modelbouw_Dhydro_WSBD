...
def make_profile(gdf):
    ...
    for i, row in tqdm.tqdm(gdf.iterrows(), total=len(gdf)):
        ...
        up_line = _make_xyz(row=row, distance_main=up_dist_main, distance_help=up_dist_help,
                            ...
                            talud_l=row['WS_TALUD_LINKS_L'], talud_r=row['WS_TALUD_RECHTS_L'], total_depth = 5)
        up_points = list(up_line.coords)
        
        for j in range(len(up_points)):
            if len(up_points[j])==3:
                new = {'geometry':Point(up_points[j]),
        ...
        for j in range(len(up_points)):
            if len(down_points[j])==3:
                new = {'geometry':Point(down_points[j]),
...
def _make_xyz(row, distance_main, distance_help, bottom_level, bottom_width, talud_l, talud_r, total_depth = 5):
