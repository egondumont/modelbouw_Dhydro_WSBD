...
class ProcessManagement:
    ...
    def run(self):
        ...
        pumpstation = gpd.read_file(self.source_data_dir / "gemaal.gpkg")
        ...
        for index_pumpstation, row_pumpstation in pumpstation.iterrows():
            ...
            if index_pumpstation not in pumpindex_hydrobjectindex:
                pumpindex_hydrobjectindex[index_pumpstation] = [None]
            pumpindex_hydrobjectindex[index_pumpstation].append(min(network.distance(row_pumpstation.geometry)))
        ...
        for index_pumpstation, row_pumpstation in pumpstation.iterrows():
            for index_management, row_management in management.iterrows():
                ...
                if row_pumpstation["code"] == row_management["pompid"]:
                    ...
                    closest_branch = network[pumpindex_hydrobjectindex[index_pumpstation]]
                    # find location on closest branch that is 3 meters upstream of the pumpstation
                    distance = closest_branch.project(row_pumpstation) - 3
                    observation_location = closest_branch.interpolate(distance)
                    observations.loc[len(observations)] = [observation_location, row_pumpstation.code, "1d"]
